...
class GIMFuCKconverter:
    ...
    def create_widgets(self):
        ...
        fmt.config(bg="#333", fg="white", highlightthickness=0)
        fmt.pack(side=tk.LEFT)

        self.btn_about = tk.Button(top, command=self.show_about, bg="#444", fg="white", relief="flat")
        self.btn_about.pack(side=tk.RIGHT, padx=10)
        self.btn_lang = tk.Button(top, command=self.show_languages, bg="#444", fg="white", relief="flat")
        ...
        sc = tk.Scrollbar(self.log_container, command=self.log_area.yview)
        ...
        # НОВАЯ КНОПКА "ВЫБРАТЬ ВСЕ"
        self.btn_select_all = tk.Button(nav, command=self.select_all_in_folder, bg="#005a9e", fg="white", relief="flat", font=("Segoe UI", 8, "bold"), padx=5)
        # Скрыта по умолчанию, показывается методом update_ui_text
    ...
    def select_all_in_folder(self):
        # Добавляем все файлы .png и .gim из текущей папки в список выбранных
        try:
            for item in self.tree.get_children():
                path = self.tree.item(item, "values")[0]
                if os.path.isfile(path) and path.lower().endswith(('.png', '.gim')):
    ...
    def update_ui_text(self):
        ...
        # Показываем/скрываем кнопку "Выбрать все"
        if self.multi_select_mode:
            self.btn_select_all.config(text=t["select_all"])
            self.btn_select_all.pack(side=tk.LEFT, padx=5, pady=2)
        else:
    ...
    def show_languages(self):
        ...
        win.title("Language"); win.geometry("250x220"); win.grab_set(); win.configure(bg="#252525")
    ...
    def show_about(self):
        t = self.langs[self.current_lang]
        win = tk.Toplevel(self.root)
        win.title(t["about"]); win.geometry("400x250"); win.configure(bg="#252525")
